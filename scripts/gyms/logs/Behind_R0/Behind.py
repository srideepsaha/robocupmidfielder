...
class InterceptionEnv(gym.Env):
    ...
    def teleport_ball_fixed(self):
        ...
        self.player.scom.unofficial_move_ball((x_pos, y_pos, 0.0), (0.0, 0.0, 0))
    ...
    def step(self, action):
        ...
        if action == 0:
            ...
            target_relative = np.array([-0.5, 0.0])
        else:
        ...
        if w.ball_rel_torso_cart_pos[0] <=0:
            if distance < 0.1:
                reward -= 0.1
...
class Train(Train_Base):
    ...
    def train(self, args):
        ...
        total_steps = 1000000
        learning_rate = 3e-4
        folder_name = f'Behind_R{self.robot_type}'
