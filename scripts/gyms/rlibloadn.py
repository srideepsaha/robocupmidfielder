...
if __name__ == "__main__":
    ...
    if not csv_path.exists():
        with open(csv_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["reward", "ep. length", "rew. cumulative avg.", "ep. len. cumulative avg."])
    ...
    for ep in range(1, NUM_EVAL_EPISODES + 1):
        ...
        while not done["__all__"]:
            ...
            # NEW: aggregate multi-agent reward (sum over agents for this step)
        ...
        # NEW: update cumulative averages
        total_rewards_sum += ep_reward
        total_lengths_sum += ep_length
        avg_rewards = total_rewards_sum / ep
        avg_lengths = total_lengths_sum / ep

        with open(csv_path, "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([ep_reward, ep_length, avg_rewards, avg_lengths])

    env.close()          # ✅ close env
    restored_algo.stop() # ✅ stop algo
    servers.kill()       # ✅ kill simulation servers
